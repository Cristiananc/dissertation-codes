...
#This would be better in a class format
def sampling_trees_complete_version(G,T_initial, n, infected_nodes, flag=0):
    """
    If we sample an observed node there are two possible operations to do, each 
    equaly likely. We either change paths from v to zero or we add a neighbor from v. 
    If the neighbor is already on the tree nothing is done. 
  
    If we sample from the list an unobserved node (that is already on the tree), we can 
    either delete them, add an unobserved neighbor or change paths. 
    """
    ...
    sampling = [T_initial]
    infecteds_to_sample = infected_nodes  
    ...
    for i in range(n):
        ...
        random_node_index = rd.randrange(0, len(infecteds_to_sample)) 
        random_node = infected_nodes[random_node_index]

        if random_node in infected_nodes:
            #Two operations can be made:
            #Add an unobserved neighbor or change the path connecting the random node to zero.
            p = np.random.uniform()
            ...
            if p < 1/2:
                ...
                #Delete the previous path from G
                #Recall that all nodes in T_current[random_node_aux][1:-1] are unobserved nodes
                if T_current[random_node_index] is not None:
                    for node in T_current[random_node_index][1:-1]:
                        G.nodes[node]['inf_time'] = math.inf
                ...
                #Check if the new path changed an unobserved node to become intermediate for 
                #their path

                if new_path is not None:
                    # Modify the current state
                    T_current[random_node_index] = new_path
                    
                    for node in new_path:
                        if node in infecteds_to_sample and node not in infected_nodes:
                            infecteds_to_sample.remove(node)
                            unobserved_leaves.remove(node)

            else:
                #Insert a new node as a leaf
                neighbors = list(G.neighbors(random_node))
                new_node_index = rd.randrange(0, len(neighbors))
                new_node = neighbors[new_node_index]

                if G.nodes[new_node]['inf_time'] == math.inf:
                    print("New node!")
                    G.nodes[new_node]['inf_time'] = G.nodes[random_node]['inf_time']

                    #Adding the new node to the tree
                    T_current.append([new_node, random_node])
                    infecteds_to_sample.append(new_node)
                    unobserved_leaves.append(new_node)

                #If the chosen new node is already on the tree, don't do anything

        #If the random selected node is an unobserved node that are on the
        # tree which is either a leaf or all their children are unobserved nodes
        # we can apply three different operations: addition of a neighbor, delete or 
        # change of paths
        elif random_node in unobserved_leaves:
            p = np.random.uniform()
            ...
            if p < 1/3:
                ...
                if G.nodes[new_node]['inf_time'] == math.inf:
                    ...
                    #Removing the random_node from unobserved_leaveas, since it
                    #is not a leaf anymore
                    unobserved_leaves.remove(random_node)
                
            #If the chosen new node is already in the tree, don`t do anything

            elif p < 2/3:
                #Changing paths
                print("Change of paths!----")
                ...
                #Delete node
                print("Deleted node")
                del T_current[random_node_index]
                G.nodes[random_node]['inf_time'] = math.inf
                infecteds_to_sample.remove(random_node)
                unobserved_leaves.remove(random_node)
